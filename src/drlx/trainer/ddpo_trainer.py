...
class DDPOExperienceReplay(Dataset):
    ...
    def __init__(
        ...
        **dataloader_kwargs,
        ...
        rewards = [
            reward_fn(img_batch, prompt_batch)
        ...
        advantages = torch.from_numpy(
            ppst.update(np.array(self.all_prompts), self.all_rewards)
        ).float()
        self.advantages = advantages.reshape(accelerator.num_processes, -1)[
            accelerator.process_index
        ].to(accelerator.device)

    def __getitem__(self, i):
        return (
            self.all_step_preds[:, i],
            self.log_probs[:, i],
            self.advantages[i],
            self.prompts[i],
        )
...
class DDPOTrainer(BaseTrainer):
    ...
    def __init__(self, config: DRLXConfig):
        ...
        assert isinstance(
            self.config.method, DDPOConfig
        ), "ERROR: Method config must be DDPO config"

        # Figure out batch size and accumulation steps
        if self.config.train.target_batch is not None:  # Just use normal batch_size
            self.accum_steps = (
                self.config.train.target_batch // self.config.train.batch_size
            )
        else:
            self.accum_steps = 1
        ...
        self.pipe = (
            None  # Store reference to pipeline so that we can use save_pretrained later
        )
    ...
    def setup_model(self):
        ...
        model = self.get_arch(self.config)(
            self.config.model, sampler=DDPOSampler(self.config.sampler)
        )
    ...
    def sample_and_calculate_rewards(
        self, prompts: Iterable[str], reward_fn: Callable
    ) -> Tuple:
        """
    ...
    def train(self, prompt_pipeline, reward_fn):
        ...
        with scoped_seed(self.config.train.seed):
            dataloader = self.accelerator.prepare(
                prompt_pipeline.create_train_loader(
        ...
        assert isinstance(
            self.sampler, DDPOSampler
        ), "Error: Model Sampler for DDPO training must be DDPO sampler"

        per_prompt_stat_tracker = PerPromptStatTracker(
            self.config.method.buffer_size, self.config.method.min_count
        )
        ...
        if self.config.train.total_samples is not None:
            outer_epochs = int(
                self.config.train.total_samples
                // self.config.train.num_samples_per_epoch
            )
        ...
        for epoch in range(outer_epochs):
            ...
            data_steps = (
                self.config.train.num_samples_per_epoch
                // self.config.train.sample_batch_size
                // self.world_size
            )
            ...
            for i, prompts in enumerate(
                ...
                batch_preds, batch_all_step_preds, batch_log_probs = self.sample(
                    prompts
                )
            ...
            all_rewards = (
                self.accelerator.gather(exp_replay.rewards).detach().cpu().numpy()
            )

            experience_loader = exp_replay.create_loader(
            ...
            with scoped_seed(self.config.train.seed):
                (
                    ...
                    _,
                    _,
                ) = self.sample_and_calculate_rewards(sample_prompts, reward_fn)
            sample_imgs = [
                wandb.Image(Image.fromarray(img), caption=prompt + f", {reward.item()}")
                for img, prompt, reward in zip(
                    sample_imgs_np, sample_prompts, sample_rewards
                )
            ]
            batch_imgs = [
                wandb.Image(Image.fromarray(img), caption=prompt)
                for img, prompt in zip(imgs[-1], all_prompts[-1])
            ]
            ...
            if self.accelerator.is_main_process and self.config.train.save_samples:
                save_images(
                    sample_imgs_np, f"./samples/{self.config.logging.run_name}/{epoch}"
                )
            ...
            for inner_epoch in tqdm(
                ...
                disable=(not self.accelerator.is_main_process)
                or (self.config.method.num_inner_epochs == 1),
            ):
                for all_step_preds, log_probs, advantages, prompts in tqdm(
                    ...
                    with self.accelerator.accumulate(
                        self.model
                    ):  # Accumulate across minibatches
                        metrics = self.loss(
                            all_step_preds, log_probs, advantages, prompts
                        )
            ...
            accum += 1
            if (
                accum % self.config.train.checkpoint_interval == 0
    ...
    def save_pretrained(self, fp: str):
        ...
        if self.accelerator.is_main_process:
            ...
            self.pipe.save_pretrained(
                fp, safe_serialization=unwrapped_model.config.use_safetensors
            )
        self.accelerator.wait_for_everyone()
